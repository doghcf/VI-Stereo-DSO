...
namespace dso
{
	...
	void Undistort::makeOptimalK_crop()
	{
		...
		// 确定经过畸变校正后图像的有效边界，即畸变校正后的图像在原始图像坐标系中的最小和最大有效区域。
	...
	void UndistortRadTan::distortCoordinates(float *in_x, float *in_y, float *out_x, float *out_y, int n) const
	{
		...
		float ofx = K(0, 0);	// K始终为单位阵，有点多余？对归一化坐标没影响
		float ofy = K(1, 1);
		float ocx = K(0, 2);
		float ocy = K(1, 2);
		...
		for (int i = 0; i < n; i++)
		{
			...
			float x_dist = ix + ix * rad_dist_u + 2.0 * p1 * mxy_u + p2 * (rho2_u + 2.0 * mx2_u);
			float y_dist = iy + iy * rad_dist_u + 2.0 * p2 * mxy_u + p1 * (rho2_u + 2.0 * my2_u);
